...
pair<PropertyState *,PropertyState *> PropertyState::split(Property * p)
{
    ...
    auto s1 = getIt(false, p, begin(), end());
    auto e1 = getIt(false, p, end(), end());

    auto s2 = getIt(true, p, begin(), end());
    auto e2 = getIt(true, p, end(), end());

    PropertyState* p1 = pmap.forceGet(s1, e1);

    if(p1==0)
    {
        p1 = new PropertyState(tan,s1.pit,e1.pit);
    };

    PropertyState* p2 = pmap.forceGet(s2, e2);

    if(p2==0)
    {
        p2 = new PropertyState(tan,s1.pit,e1.pit);
    };
